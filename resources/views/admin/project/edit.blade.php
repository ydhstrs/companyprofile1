            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="block w-full overflow-x-auto p-8">
                        <form method="post" action="/dashboard/project/{{ $project->slug }}" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="mb-6">
                                <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Title</label>
                                <input type="text" id="title" name="title"
                                    class="block w-full p-2.5 bg-gray-50 border border-gray-300 text-black text-sm rounded-lg "value="{{ $project->title }}">
                            </div>
                            <div class="mb-6">
                                <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Slug</label>
                                <input type="text" id="slug" name="slug"
                                    class="block w-full p-2.5 bg-gray-50 border border-gray-300 text-black text-sm rounded-lg " value="{{ $project->slug }}">
                            </div>
                    <div class="mb-6">
                        @if ($project->image)
                            <img src="{{ asset('/storage/' . $project->image) }}" class="img-preview w-56">
                        @else
                        ...
                        @enderror
                            ...
                            <div class="mb-6">
                                <label class="block">
                                    ...
                                    <textarea id="editor" class="block w-full mt-1 rounded-md" name="isi" rows="3">{{ $project->isi }}</textarea>
                                </label>
                                @error('description')
