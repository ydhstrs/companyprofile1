                        ...
                        <form method="post" action="/dashboard/article/{{ $article->slug }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="text" id="title" name="title"
                                    class="block w-full p-2.5 bg-gray-50 border border-gray-300 text-black text-sm rounded-lg "value="{{ $article->title }}">
                            </div>
                            <div class="mb-6">
                                <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Slug</label>
                                <input type="text" id="slug" name="slug"
                                    class="block w-full p-2.5 bg-gray-50 border border-gray-300 text-black text-sm rounded-lg " value="{{ $article->slug }}">
                            </div>
                    <div class="mb-6">
                        @if ($article->image)
                            <img src="{{ asset('/storage/' . $article->image) }}" class="img-preview w-56">
                        @else
                        ...
                        @enderror
                            ...
                            <div class="mb-6">
                                <label class="block">
                                    ...
                                    <textarea id="editor" class="block w-full mt-1 rounded-md" name="isi" rows="3">{{ $article->isi }}</textarea>
                                </label>
                                @error('description')
